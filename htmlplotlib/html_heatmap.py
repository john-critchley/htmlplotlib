...
def generate_horizontal_color_bar(cmap_name: str, width: str = '100%',
    ...
    label_texts = labels or [f'{tick:.1f}' for tick in np.linspace(0, 1, num_labels)]  # Corrected labels from 0 to 1
    label_css = '; '.join(f'{k}: {v}' for k, v in (label_style or {}).items())

    tick_html = (
        f'<div style="position: absolute; left: {pos * 100:.2f}%; height: 100%; width: 1px; '
        f'background-color: white;"></div>'
        for pos in np.linspace(0, 1, num_labels)
    )

    label_html = (
        f'<span style="position: absolute; left: {pos * 100:.2f}%; top: calc(100% + 3px); '
        f'transform: translateX(-50%); font-size: 10px; {label_css}">{text}</span>'  # Reduced label size
        for text, pos in zip(label_texts, np.linspace(0, 1, num_labels))  # Updated to correctly position labels
    )

    html = (
        f'<div style="position: relative; width: {width}; height: {height}; margin-top: 20px; margin-left: 40px;">'  # Adjusted width and margins dynamically
        f'<div style="width: 100%; height: 100%; background: linear-gradient(to right, {colors});"></div>'  # Corrected gradient direction
        f'<div style="position: absolute; top: 0; width: 100%; height: 100%;">'
        f'{"".join(tick_html)}'
        f'</div>'
        f'{"".join(label_html)}'
        f'</div>'
